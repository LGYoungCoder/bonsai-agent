  ...
  if (msg.cmd === 'management') {
    try {
      ...
      if (msg.method === 'reload') {
        chrome.alarms.create('bonsai-bridge-self-reload', { when: Date.now() + 200 });
        return { ok: true };
      }
...
// --- WebSocket Client for Bonsai bridge ---
let ws = null;
const WS_URL = 'ws://127.0.0.1:18765';

function scheduleProbe() {
  // Use chrome.alarms to survive MV3 service worker suspension
  chrome.alarms.create('bonsai-bridge-ws-probe', { delayInMinutes: 0.083 }); // ~5s
}

function scheduleKeepalive() {
  // Keep SW alive while WS is connected (~25s, under 30s SW timeout)
  chrome.alarms.create('bonsai-bridge-ws-keepalive', { delayInMinutes: 0.4 }); // ~24s
}
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  if (alarm.name === 'bonsai-bridge-self-reload') {
    chrome.runtime.reload();
    return;
  }
  if (alarm.name === 'bonsai-bridge-ws-keepalive') {
  ...
  if (alarm.name === 'bonsai-bridge-ws-probe') {
    if (ws && ws.readyState <= 1) return; // Already connected/connecting
    if (await isServerAlive()) {
      console.log('[bonsai-bridge] Server detected, connecting...');
      connectWS();
    } else {
...
async function handleWsExec(data) {
  ...
  console.log('[bonsai-bridge] Exec request', data.id, 'on tab', tabId);
  ...
  try {
    ...
    try {
      ...
      if (res === null || res === undefined) {
        console.log('[bonsai-bridge] executeScript returned null/undefined, treating as CSP issue');
        res = { ok: false, error: { name: 'Error', message: 'executeScript returned null (possible CSP or context issue)', stack: '' }, csp: true };
      }
    } catch (e) {
      console.log('[bonsai-bridge] scripting.executeScript failed:', e.message);
    ...
    if (res && !res.ok && res.csp) {
      console.log('[bonsai-bridge] CDP fallback for tab', tabId);
...
function connectWS() {
  ...
  ws = null;
  console.log('[bonsai-bridge] Connecting to', WS_URL);
  try {
    ws = new WebSocket(WS_URL);
  } catch (e) {
    console.error('[bonsai-bridge] Constructor error:', e);
    ws = null;
    scheduleProbe();
    return;
  }
  ws.onopen = async () => {
    console.log('[bonsai-bridge] Connected!');
    ...
    console.log('[bonsai-bridge] Sent ext_ready with', tabs.length, 'tabs');
  };
  ws.onmessage = async (event) => {
    try {
      ...
      console.error('[bonsai-bridge] message parse error', e);
    }
  };
  ws.onclose = () => {
    console.log('[bonsai-bridge] Disconnected');
    ws = null;
    scheduleProbe();
  };
  ws.onerror = (e) => {
    console.error('[bonsai-bridge] Error:', e);

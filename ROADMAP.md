...
**v0.4 总计 ≈ 7600 行**。对比:
- 典型轻量 agent(含浏览器 + 前端): ~8-10K 行
- 纯记忆栈(向量 + BM25 + verbatim): ~3K 行
- OpenHands / AutoGPT: 10 万行+

**Bonsai 定位**:Agent 主循环 + verbatim memory + 多 provider + 多前端 + 浏览器,做进**更少的**代码里。克制的含义是**"每一行都有理由"**,不是"少到无法运作"。

...
装在你自己的电脑上，用你自己的 API key，越用越懂你。聊天记录、记忆库、技能、配置——全部落在你的盘上，不上任何云、不被任何平台分析。它眼里只有一个人，就是你。

它的 6 个特点：

- **只认你一个人**：记得你的习惯、项目、口头禅，跨天跨设备都接得上；不区分"用户"，没有用户表，连登录都不需要
- **数据握在你手里**：聊天 / 记忆 / 配置全部落在你的盘里，整个 bonsai 目录拷走就是完整迁移
- **API key 是你的**：用多少花多少，不订阅、不抽成，每一笔花在哪条对话上都看得见
- **能动手**：可以打开浏览器、改文件、跑代码，不只是回复文字
- **跑在你说了算的地方**：笔记本、家里的小服务器、VPS 都行，想停就停想搬就搬
- **挂得进 IM**：微信、QQ、Telegram、飞书、钉钉，五选一或全选

**一句话**：它不是给"用户"的产品，是给**你**的搭档。扔到你的服务器上 7×24 跑着，你在 QQ 上问、微信上问、电脑上问，都是同一个它，记着同一份过往。
